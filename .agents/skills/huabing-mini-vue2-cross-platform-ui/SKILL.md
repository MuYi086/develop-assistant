...
description: "话饼跨端 UI 和组件技能。用于构建或修复 Vue 2 uni-app 的 hb-* 组件、页面局部组件、uv-ui 二次封装、自定义导航、按钮、弹窗、购物车、swiper、vtabs、sticky 和 scroll 布局，以及微信小程序、支付宝小程序、H5 兼容问题。"
---
...
参考 `src/components/hb-navbar/hb-navbar.vue`：

- 用 `uni.getSystemInfoSync()` 和 `uni.getMenuButtonBoundingClientRect()` 计算状态栏、胶囊和内容宽度。
...
参考 `src/components/hb-button/hb-button.vue`：

- H5、微信、支付宝 open-type 行为用不同模板。
...
参考 `src/pagesA/shop/components/hb-uv-vtabs.vue`：

- 先测量 section 高度和 top，再在滚动中使用缓存结果。
- 重计算布局要防抖。
- 左侧菜单滚动和右侧内容滚动分开处理。
...
- `src/components/hb-shopping-cart-btn/hb-shopping-cart-btn.vue`
- `src/components/hb-shopping-cart-popup/hb-shopping-cart-popup.vue`
- `src/pagesF/orderConfirm/components/*`
